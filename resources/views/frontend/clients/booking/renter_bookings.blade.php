<ul>
    @foreach($renterBookings as $booking)
        <li class="booking-{{$booking->booking_status_id}}">
            ...
            @if($booking->booking_status_id==5 || $booking->booking_status_id==4)
                <a href="{{route('frontend.camper.detail.booked',$booking->id_campers)}}" class="button gray approve"><i class="far fa-eye"></i> {{trans('front.check_camper')}}</a>
            @endif
