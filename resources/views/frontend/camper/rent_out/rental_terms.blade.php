...
<div class="dashboard-content">
	...
	<div class="row">
		...
		<form name="formRentalTerms" action="{{route('frontend.camper.storeterms')}}" method="POST" onSubmit="return validateForm()">
		@csrf
			<input type="hidden" name="id_campers" value="{{$camper->id}}" />
			<div class="col-lg-7 col-md-12">
				<h3><strong>{{trans('front.rental_terms')}}</strong></h3>
				<div class="col-md-12">
				<div id="error-valid" class="notification danger" style="display:none; width: 100% !important">
					<p><span>{{trans('front.error')}}!</span> {{trans('front.hirer_licence_required')}}</p>
					<a class="close"></a>
				</div>
					<div class="col-md-6">
					...
					<div class="col-md-6">
						...
						<div class="col-md-5" style="padding-top: 12px;">
							<label class="containerRadio">{{trans('front.yes')}}
								<input type="radio" name="animals_allowed" value="yes" required @if($camper->animals_allowed=="yes") checked="checked" @endif>
...
function validateForm() {
	var errors = [];
	var form = document.getElementsByTagName('formRentalTerms')[0];
	...
	*/
	var fnElem = document.getElementById("minimum_age");
	var lnElem = document.getElementById("license_age");
	var minimum_age = fnElem.options[fnElem.selectedIndex].value;
	var license_age = lnElem.options[lnElem.selectedIndex].value;


	if(minimum_age === "" || license_age === ""){
